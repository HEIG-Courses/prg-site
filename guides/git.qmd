...
rôles sont inversés, `HEAD` est le travail publié par votre binôme et le
bloc du dessous est votre commit en train d'être rejoué ; on termine alors
par `git rebase --continue`. Pour tout annuler et revenir à l'état d'avant :
`git rebase --abort`.
...
une demande d'intégrer la branche dans `main`, que le binôme relit avant
de fusionner. Deux raisons : une relecture avant que le code n'entre dans
`main`, et un historique lisible, un commit par fonctionnalité. Le
déroulement complet :

1. Créer la branche et s'y placer :

   ```sh
   git checkout -b saisie-nom
   ```

2. Travailler, puis valider comme d'habitude :

   ```sh
   git add main.cpp
   git commit -m "Ajouter la saisie du nom"
   ```
...
7. Fusion : le bouton vert propose trois méthodes dans son menu. **Create
   ...
   message proposé, confirmez.

8. Cliquez **Delete branch** : la branche a rempli son rôle.
...
**Create Pull Request** ; le reste se passe sur GitHub. Rebase et squash
sont décrits dans [Pour aller plus loin](#rebase-squash).
...
::: {#rebase-squash .callout-note collapse="true" title="Rebase et squash"}
...
réduit les commits de la branche à un seul avant de fusionner.
:::
