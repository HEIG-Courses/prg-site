...
2. La **staging area** : les changements que vous avez marqués pour le
   ...
   messages.
3. Le **dépôt local** : l'historique validé (`git commit`), sur votre
   ordinateur.
...
Rien n'est cassé : votre binôme a poussé sur `main` avant vous, et les deux
copies ont divergé.
...
La séquence de récupération, grâce au réglage `pull.rebase` du
[guide d'installation](installation.qmd#configuration-de-git), qui rejoue
vos commits par-dessus les leurs :
...
la branche ; comme elle est maintenant dans le prolongement de `main`,
`main` avance simplement jusqu'à elle, sans commit de fusion.
